...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="kimwk0229/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/kimwk0229/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/kimwk0229/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://kimwk0229:${GITHUB_ACCESS_TOKEN}@github.com/kimwk0229/guestbook-config.git"
                    '''
                }
            }
        }
